...
export default function Cadence() {
  ...
  useEffect(() => {
    let active = true;
    const updateNotice = () => setNotice(storageNotice());
    window.addEventListener(STORAGE_EVENT, updateNotice);
    void (async () => {
      const [bank, savedStats, savedProfile, savedLearning] = await Promise.all(
        [
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-5 sm:py-8">
        ...
        {notice && (
          <div
            role="status"
            className="mb-5 flex gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-900"
          >
